...
def install_chr2_pyrho6(cell: CellFromNetPyNE, rows_template, gbar_mS_cm2: float, irradiance_scale: float, rate_scale: float):
...
def simulate_pyrho6_current(
    ...
    rows = install_chr2_pyrho6(cell, rows_template, gbar_mS_cm2, irradiance_scale, rate_scale)
...
def calibrate_pyrho6_gbar(cell_params, rows, args, protocol, rate_scale: float):
    lo = 1.0e-5
    hi = 2.0
    last = None
...
def run_trace(cell_params, rows, gbar, irradiance, pulse_duration_ms, tstop_ms, args, protocol, rate_scale):
...
def main():
    ...
    for q10 in parse_floats(args.q10_values):
        ...
        gbar, calib_peak = calibrate_pyrho6_gbar(args.cell_params, rows, args, protocol, rate_scale)
        trace_1s = run_trace(args.cell_params, rows, gbar, protocol.irradiance_mw_mm2, 1000.0, 1200.0, args, protocol, rate_scale)
        traces[q10] = trace_1s
        ...
        for irr in WANG_INTENSITY_VALUES_MW_MM2:
            trace = run_trace(args.cell_params, rows, gbar, irr, 100.0, 180.0, args, protocol, rate_scale)
            ...
            filt_ttps.append(filt_ttp_i)
            filt_steadies.append(filt_steady_i)
    ...
    for q10, trace in traces.items():
        axes[0].plot(trace["t"], -trace["filtered_i"], lw=1.2, label=f"Q10={q10:g}")
    ...
    for row in summary_rows:
        q10 = row["q10"]
        sub = [item for item in intensity_rows if item["q10"] == q10]
        axes[1].plot(
            [item["irradiance_mw_mm2"] for item in sub],
            [item["filtered_peak_nA"] for item in sub],
            marker="o",
            label=f"Q10={q10:g}, K={row['filtered_intensity_k_mw_mm2']:.2g}",
        )
    ...
    axes[2].set_xticks(x, [f"Q10={row['q10']:g}" for row in summary_rows])
